...
export const getListProvincias = async () => {
  const response = await axiosPrivate.get(`${CONTROLLER}listProvincias`);
  return response;
};
...
export const listProductsFavorites = async (nickname: string) => {
  const response = await axiosPrivate.get(
    `${CONTROLLER}${nickname}/listProductsFavorites`
  );
  return response;
};
...
export const removeFavorite = async (nickname: string, productId: number) => {
  const response = await axiosPrivate.get(
    `${CONTROLLER}${nickname}/removeFavorite/${productId}`
  );
  return response;
};
...
export const getUser = async (nickname: string) => {
  const response = await axiosPrivate.get(`${CONTROLLER}${nickname}`);
  return response;
};
